...
"""
def create_circle(arr: list, center: tuple, rad: int, alf: int, alf_step: tuple, spread: tuple, heightmap, image, output_arr: list):
    ...
    while alf < 628 - alf_step[1]*0.8:
        ...
        try:
            ...
            output_arr[-1][1] = get_height(heightmap, x, y, can_down=down)
...
def give_size(arr: list, image):
    ...
    #Вишня 3 острова
    quests = [cur for cur in arr if (30 <= (cur[0] ** 2 + cur[2] ** 2) ** 0.5 < 500) and is_normal_height(cur[1])]
    for x in sample(range(0, len(quests)), 3):
        quests[x][-1] = 50

    #Старый остров (используются значения как и у морского царя)
    #Морской царь
    quests = [cur for cur in arr if (300 <= (cur[0] ** 2 + cur[2] ** 2) ** 0.5 <= 750) and is_normal_height(cur[1]) and cur[3] != 50]
